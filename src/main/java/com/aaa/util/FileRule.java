...
public class FileRule {
    ...
    //username :用户名
    //staticurl:配置的本地磁盘
    public  String fileupload(String staticurl ,MultipartFile file) throws IOException {
        ...
        if(!filename.isEmpty()){
            //判断用户名子文件是否存在
            File fileusername=new File(staticurl);
            ...
            if(Arrays.asList(photoformat).contains(filesuf)){
                File filephoto=new File(staticurl+"/photo");
                //判断filephoto文件加是否存在
                if(!filephoto.exists()){
                    //创建目录
                    filephoto.mkdirs();
                ...
                uploadstr="http://localhost:8089/ailybg/staticfile"+"/photo/"+photo;
                File newfile = new File(staticurl+"/photo/"+photo);
                //文件做提交
