...
public interface PositionDao extends tk.mybatis.mapper.common.Mapper<Position> {
    @Select("select p.pos_id,p.pos_name,p.ind_id,i.ind_name,p.pos_state from position p join industry i on p.ind_id=i.ind_id")
    List<Map<String,Object>> pos_query();
    @Insert(value = "insert into position (pos_name,ind_id,pos_state) values (#{pos_name},#{ind_id},1)")
